...
LANG_TO_HF_REPO_ID = {
    'EN': 'models/English',
    'EN_V2': 'models/English-v2',
    'EN_NEWEST': 'models/English-v3',
}

def load_or_download_config(locale):
    return get_hparams_from_file(f'models/{locale}/config.json')

def load_or_download_model(locale, device):
    return torch.load(f'models/{locale}/checkpoint.pth', map_location=device)

class TTS(nn.Module):
    def __init__(self, language, device='auto'):
        super().__init__()
